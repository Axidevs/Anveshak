...
const mockTarget = `  const totalCases = mockOfficerCases.length;
...
const newTarget = `  const [stats, setStats] = React.useState({ total: 0, active: 0, pending: 0, shared: 0 });
...
code = code.replace(mockTarget, newTarget);
...
code = code.replace(`{c.caseId}`, `{c.caseId || c._id}`);
