...
const start = lines.findIndex(l => l.includes('const fetchCase = async () => {'));
const end = lines.findIndex((l, i) => i > start && l.includes('fetchCase();'));
...
lines.splice(start, end - start, newFetch);
fs.writeFileSync('src/pages/officer/CaseDetail.jsx', lines.join('\n'));
console.log("Patched fetchCase perfectly!");
