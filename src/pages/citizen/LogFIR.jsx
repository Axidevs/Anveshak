...
const LogFIR = () => {
  ...
  const getFileIcon = (type) => {
    if (type.includes('image')) {
      return <ImageIcon size={20} className="text-saffron" />;
    }

    if (type.includes('pdf')) {
      return <FileText size={20} className="text-alert" />;
    }

    return <File size={20} className="text-navy" />;
  };

  // REAL BACKEND FIR SUBMISSION
  const handleSubmit = async () => {
    ...
    try {
      const firData = {
        complainant: 'Registered Citizen',
      ...
      // 1. Save FIR in MongoDB
      const firResponse = await apiFetch('/fir', {
        method: 'POST',
        body: JSON.stringify(firData)
      });

      console.log('FIR created:', firResponse);

      // 2. Create Case from the FIR
      const firId =
        firResponse?.fir?._id ||
        firResponse?.fir?.id ||
        firResponse?._id ||
      ...
      if (!firId) {
        throw new Error('FIR created but FIR ID was not returned');
      }
      ...
      // 3. Show actual FIR / Case identifier
      const generatedCaseId =
        caseResponse?.case?.caseId ||
        caseResponse?.caseId ||
        firResponse?.fir?.firNumber ||
        firResponse?.firNumber;

      setTrackingId(generatedCaseId || 'CASE CREATED');
      setCurrentStep(5);
    } catch (error) {
      console.error('FIR submission error:', error);
      alert(error.message || 'Failed to submit FIR. Please try again.');
    } finally {
  ...
  // DOWNLOAD FIR COPY
  const downloadFIRCopy = () => {
    const firText = `
ANVESHAK - FIR COPY

Case / FIR ID: ${trackingId}

Incident Type:
${incidentType || 'Not specified'}

Date & Time:
${incidentDate || 'N/A'} at ${incidentTime || 'N/A'}
...
This FIR has been securely submitted through Anveshak.
`;

    const blob = new Blob([firText], {
    ...
    link.href = url;
    link.download = `${trackingId || 'Anveshak-FIR'}-FIR-Copy.txt`;
  ...
  const incidentTypes = [
    ...
    {
      id: 'assault',
      label: t('assault') || 'Assault',
      icon: <UserX size={32} />
    },
    {
      id: 'fraud',
      label: t('fraud') || 'Fraud / Cyber',
      icon: <AlertTriangle size={32} />
    },
    {
      id: 'missing',
      label: t('missing') || 'Missing Person',
      icon: <Search size={32} />
    },
    {
      id: 'accident',
      label: t('accident') || 'Accident',
      icon: <Car size={32} />
    },
    {
      id: 'domestic',
      label: t('domestic') || 'Domestic Violence',
      icon: <Home size={32} />
    },
    {
      id: 'property',
      label: t('property') || 'Property Dispute',
                ...
                <label className="block text-sm font-medium text-gray-700 mb-2 flex justify-between">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 text-sm font-medium transition-colors ${
                      isListening ? 'text-alert' : 'text-navy'
                    }`}
                  >
                    {isListening ? (
                      <>
                ...
                <div className="h-48 bg-gray-100 rounded-lg border border-gray-300 flex items-center justify-center relative overflow-hidden">
                  ...
                  {coordinates ? (
                    <div className="text-center">
                      <MapPin
                        size={32}
                        className="text-alert mx-auto mb-2"
                      />
            ...
            <div className="animate-fade-in">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-navy hover:bg-navy-50 transition-colors relative">
                ...
                <p className="text-xs text-gray-400">
                  Supports Images, Videos, PDF (Max 5 files)
                </p>
                    ...
                    {files.map((file, index) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-medium text-charcoal truncate max-w-[200px] sm:max-w-xs">
                            {file.name}
                          </span>
          ...
          {currentStep === 5 && (
            <div className="animate-fade-in text-center py-10">
              ...
              <p className="text-gray-600 mb-8">
                Your report has been securely transmitted to the jurisdiction station.
              </p>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 max-w-sm mx-auto mb-8">
                ...
                <p className="text-2xl font-bold text-navy tracking-wider mb-6">
                  {trackingId}
                </p>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <button
                  onClick={downloadFIRCopy}
                  className="w-full sm:w-auto px-6 py-3 bg-white border-2 border-navy text-navy rounded-lg font-bold hover:bg-navy-50 transition-colors flex items-center justify-center gap-2"
                >
                  <FileText size={20} />
                  Download FIR Copy
                </button>
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            {currentStep < 4 ? (
              ...
              <button
                ...
                className={`px-8 py-3 bg-forest text-white rounded-lg font-bold transition-all shadow-sm flex items-center gap-2 ${
                  isSubmitting
