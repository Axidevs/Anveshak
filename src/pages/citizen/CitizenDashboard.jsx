...
const CitizenDashboard = () => {
  ...
  useEffect(() => {
    const loadFIRs = async () => {
      try {
        ...
        setFirs(Array.isArray(firData) ? firData : []);
      } catch (error) {
        console.error('Failed to load citizen FIRs:', error);
        setFirs([]);
      } finally {
        setLoading(false);
      }
  ...
  const resolvedCases = firs.filter(
    (fir) => fir.status === 'CLOSED'
  ).length;

  const pendingAction = firs.filter(
    (fir) =>
      fir.status === 'SUBMITTED' ||
      fir.status === 'UNDER_REVIEW'
  ).length;
  ...
  const stats = [
    {
      label: t('totalFirsFiled') || 'Total FIRs Filed',
      value: totalFIRs,
    ...
    },
    {
      ...
      value: activeCases,
      icon: <Activity size={24} className="text-saffron" />,
      bgColor: 'bg-saffron-50'
    },
    {
      label: t('resolvedCases') || 'Resolved',
      value: resolvedCases,
      icon: <CheckCircle size={24} className="text-forest" />,
      bgColor: 'bg-green-50'
    },
    {
      label: t('pendingAction') || 'Pending Action',
      value: pendingAction,
      icon: <AlertCircle size={24} className="text-alert" />,
      bgColor: 'bg-red-50'
    }
  ];

  const recentFIRs = firs.slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 mb-8">
          ...
          <p className="text-gray-600 mt-2">
            {t('dashboardDescription') ||
              'Manage your FIRs and track justice progress digitally.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {stats.map((stat, index) => (
            <div
              key={index}
              className={`rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-6 bg-white border-l-4 ${
                index === 0
                  ? 'border-navy'
                  : index === 1
                  ? 'border-saffron'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          ...
          <Link to="/citizen/log-fir" className="group">
            <div className="bg-navy rounded-xl p-8 flex items-center justify-between shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 text-white h-full">
              ...
              <div>
                ...
                <p className="text-navy-100 opacity-90">
                  File a new First Information Report completely online.
                </p>
              </div>

              <ArrowRight
                size={32}
                className="group-hover:translate-x-2 transition-transform opacity-70 group-hover:opacity-100"
              />
            </div>
          </Link>
          ...
          <Link to="/citizen/view-firs" className="group">
            <div className="bg-white border-2 border-navy rounded-xl p-8 flex items-center justify-between shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 h-full text-navy">
              ...
              <ArrowRight
                size={32}
                className="group-hover:translate-x-2 transition-transform text-navy opacity-70 group-hover:opacity-100"
              />
            </div>
          </Link>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              recentFIRs.map((fir) => (
                <div
                  key={fir._id || fir.id || fir.firNumber}
                  className="border border-gray-100 rounded-lg p-4 hover:bg-gray-50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                >
                  <div>
                    ...
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                        ID: {fir.firNumber || fir.trackingId || fir._id}
                      </span>
