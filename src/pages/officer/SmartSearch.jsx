...
export default function SmartSearch() {
  ...
  const breadcrumbs = [
    ...
    {
      label: t('Smart Search') || 'Smart Search',
      path: '/officer/search'
    }
  ];
    ...
    const loadCases = async () => {
      try {
        ...
        const normalizedCases = rawCases.map((item) => {
          const assignedOfficer = item.assignedOfficer;
          ...
          const type =
            item.type ||
            item.category ||
            item.aiAnalysis?.classification ||
            'General';

          return {
            id: item.caseId || item.id || 'N/A',
            title:
              item.title ||
              item.caseTitle ||
              'Case Record',
        ...
        console.error('Smart Search cases load failed:', error);
        setCases([]);
      }
    };
  ...
  const filteredCases = cases.filter((c) => {
    ...
    switch (activeFilter) {
      ...
      case 'Department':
        filterMatch = c.department
          .toLowerCase()
          .includes(filterValLower);
        break;

      case 'Status':
        filterMatch = c.status
          .toLowerCase()
          .includes(filterValLower);
        break;
      ...
      case 'Keyword':
        filterMatch =
          c.title.toLowerCase().includes(filterValLower) ||
          c.type.toLowerCase().includes(filterValLower) ||
          c.id.toLowerCase().includes(filterValLower);
        break;
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="relative">
          ...
          <input
            ...
            placeholder={
              t('Search cases, documents, or patterns...') ||
              'Search cases, documents, or patterns...'
            }
            value={searchTerm}
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {filteredCases.length === 0 && (
                <tr>
                  ...
                  >
                    {t('No cases found matching your search.') ||
